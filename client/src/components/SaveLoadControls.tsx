...
export const SaveLoadControls: React.FC<SaveLoadControlsProps> = ({ onSave, onLoad }) => {
  ...
  return (
    <div className="save-load-controls">
      <button className="control-btn save-btn" onClick={() => setShowDialog('save')} title="Save World (Ctrl+S)">
        💾 Save
      </button>
      <button className="control-btn load-btn" onClick={() => setShowDialog('load')} title="Load World (Ctrl+L)">
        📂 Load
      </button>
        ...
        <div className="dialog-overlay" onClick={() => setShowDialog(null)}>
          <div className="dialog" onClick={(e) => e.stopPropagation()}>
            <div className="dialog-header">
              <h3>Save World</h3>
              <button className="close-btn" onClick={() => setShowDialog(null)}>×</button>
            </div>
                ...
                {saves.length === 0 ? (
                  ...
                  <ul>
                    {saves.map(save => (
                      <li key={save}>
                        <span>{save}</span>
                        <button onClick={() => handleDelete(save)} className="delete-btn">🗑️</button>
                      </li>
                    ))}
                  </ul>
                )}
            ...
            <div className="dialog-actions">
              <button onClick={() => setShowDialog(null)} disabled={loading}>Cancel</button>
              <button onClick={handleSave} disabled={loading || !newSaveName.trim()} className="primary">
                {loading ? 'Saving...' : 'Save'}
              </button>
            </div>
          </div>
              ...
              {saves.length === 0 ? (
                ...
                <div className="save-list">
                  {saves.map(save => (
                    ...
                    >
                      <span>📄 {save}</span>
                      <button onClick={(e) => { e.stopPropagation(); handleDelete(save); }} className="delete-btn">🗑️</button>
                    </div>
                  ))}
                </div>
              )}
