...
class EconomicPlanner:
    ...
    def evaluate_craft_path(
        self, item: str, npc_state: Dict[str, Any], world_state: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def get_acquisition_plan(
        ...
        # Debug output
        print(
            f"  [DEBUG] Craft: time={craft_eval['time_cost']:.1f}, success={craft_eval['success_probability']:.2f}, quality={craft_eval['expected_quality']:.2f}, total_cost={craft_eval['total_cost']:.1f}"
        )
        print(
            f"  [DEBUG] Work: time={work_eval['time_cost']:.1f}, guaranteed=True, total_cost={work_eval['total_cost']:.1f}"
        )
        print(
            f"  [DEBUG] Time ratio: {time_ratio:.2f}x, Risky: {craft_risky}, Low quality: {craft_low_quality}"
        )
        ...
        if craft_eval["success_probability"] < 0.5 or craft_eval["expected_quality"] < 0.4:
            print("  [DEBUG] → work_and_buy (very risky/low quality)")
            return ("work_and_buy", work_eval)
        ...
        if (craft_risky or craft_low_quality) and time_ratio < 3.0:
            print("  [DEBUG] → work_and_buy (risky and work not too slow)")
            return ("work_and_buy", work_eval)
