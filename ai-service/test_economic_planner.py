...
def test_craft_vs_work_decision():
    ...
    print(
        f"   Reason: Crafting success={details.get('success_probability', 'N/A')}, quality={details.get('expected_quality', 'N/A')}"
    )
    ...
    print(f"   Strategy: {strategy}")
    print(
        f"   Reason: Crafting success={details.get('success_probability', 'N/A'):.2f}, quality={details.get('expected_quality', 'N/A'):.2f}"
    )
    ...
    strategy, details = planner.get_acquisition_plan(
        "sword", npc_medium_craft_high_gather, world_state
    )
...
def test_work_calculation():
    ...
    npc_state = {"skills": {"crafting": 30, "gathering": 80, "trading": 40}, "stats": {"money": 20}}

    world_state = {"market_prices": {"expensive_item": 100}, "quest_board": []}

    strategy, details = planner.get_acquisition_plan("expensive_item", npc_state, world_state)
    print("\n💰 Work Calculation Test:")
    print("   Item price: 100 gold")
    print("   Current gold: 20")
