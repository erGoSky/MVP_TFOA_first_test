...
class TestGoalGenerator(unittest.TestCase):
    ...
    def test_hunger_goal_generation(self):
        ...
        has_eat_goal = any(g.type == GoalType.MAINTAIN_NEED and g.id.startswith('eat_food') for g in goals)
    ...
    def test_sleep_goal_generation(self):
        ...
        has_sleep_goal = any(g.type == GoalType.MAINTAIN_NEED and g.id.startswith('rest') for g in goals)
    ...
    def test_no_goals_when_satisfied(self):
        ...
        self.assertEqual(len(critical_goals), 0, "Should not generate critical goals when satisfied")
