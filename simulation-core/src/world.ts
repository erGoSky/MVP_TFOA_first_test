...
export class WorldManager {
  ...
  public createNPC(
    ...
    console.log(
      `Created NPC ${name} with personality: ${PersonalityGenerator.describe(npc.personality)} (${npc.personality.archetype})`
    );
  }
  ...
  private async decideAction(npc: NPC) {
    try {
      ...
      if (bestAction) {
        console.log(
          `NPC ${npc.name} decided to: ${bestAction.name} (U=${response.data.utility.toFixed(2)})`
        );
  ...
  private getCraftingOptions(npc: NPC) {
    ...
    const hasWood = npc.inventory.some(
      (i) => woodItems.includes(i.type) && this.ITEM_REGISTRY[i.type]?.includes("craft")
    );
    ...
    const hasWoodForChest = npc.inventory.some(
      (i) => ["wood", "plank"].includes(i.type) && i.quantity >= 4
    );
  ...
  public tick() {
    ...
    Object.values(this.state.npcs).forEach((npc) => {
      ...
      const visibleEntities = Object.values(this.state.entities).filter(
        (e) => this.getDistance(npc.position, e.position) <= 10 // Visual range
      );
