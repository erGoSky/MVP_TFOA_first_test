...
export class ResourceHandler implements ActionHandler {
  execute(npc: NPC, actionType: string, targetId: string, world: WorldManager): void {
    ...
    if (actionType === "chop") {
      if (
        resource &&
        resource.resourceType.includes("tree") &&
        world.getDistance(npc.position, resource.position) <= 1.5
      ) {
      ...
      if (
        resource &&
        (resource.resourceType.includes("rock") || resource.resourceType.includes("ore")) &&
        world.getDistance(npc.position, resource.position) <= 1.5
      ) {
        // Determine drop based on resource type
        const dropType =
          resource.resourceType === "rock_stone"
            ? "stone"
            : resource.resourceType === "ore_iron"
              ? "ore_iron"
              : resource.resourceType === "ore_coal"
                ? "ore_coal"
                : "stone";
