    ...
    if (resource && world.getDistance(npc.position, resource.position) <= 1.5) {
      ...
      if (category === "liquid") {
        ...
        if (npc.hands && npc.hands.type.includes("bucket") && !npc.hands.contents?.length) {
          // Fill bucket
          npc.hands.contents = [
            { id: `fluid_${Date.now()}`, type: resource.resourceType, quantity: 10 },
          ]; // Assume full bucket
          success = true;
        } else {
        ...
        const currentCount = npc.inventory
          .filter((i) => i.type === resource.resourceType)
        ...
        if (currentCount + pickupAmount > 3) {
          // Check for sack
          const sack =
            npc.inventory.find((i) => i.type === "sack") ||
            (npc.hands?.type === "sack" ? npc.hands : null);
          if (sack) {
            // Add to sack
