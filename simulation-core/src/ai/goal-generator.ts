    ...
    static generateFromNeeds(npc: NPC, worldState: WorldState, currentTick: number): Goal[] {
        ...
        if (npc.needs.hunger > 0.7) {
            goals.push({
                ...
                priority: 0.9,
                conditions: [
                    { type: 'stat_value', key: 'hunger', value: 0.3, operator: '<=' }
                ],
                isGlobal: false,
        ...
        if (npc.needs.energy < 0.3) {
            goals.push({
                ...
                priority: 0.85,
                conditions: [
                    { type: 'stat_value', key: 'energy', value: 0.8, operator: '>=' }
                ],
                isGlobal: false,
                createdAt: currentTick
            });
        ...
        if (npc.stats.health < 50) {
            goals.push({
                ...
                priority: 0.95,
                conditions: [
                    { type: 'stat_value', key: 'health', value: 80, operator: '>=' }
                ],
                isGlobal: false,
                createdAt: currentTick
            });
        ...
        if (npc.stats.money < 10) {
            goals.push({
                ...
                priority: 0.6,
                conditions: [
                    { type: 'stat_value', key: 'money', value: 50, operator: '>=' }
                ],
                isGlobal: true,
                createdAt: currentTick
            });
    ...
    static generateContextGoals(npc: NPC, worldState: WorldState, currentTick: number): Goal[] {
            ...
            case NPCContextState.DESPERATE:
                ...
                if (npc.needs.hunger > 0.8) {
                    goals.push({
                        ...
                        priority: 1.0,
                        conditions: [
                            { type: 'has_item', key: 'food', value: 1, operator: '>=' }
                        ],
                        isGlobal: false,
                        createdAt: currentTick
                    });
            ...
            case NPCContextState.THRIVING:
                ...
                if (npc.stats.money >= 100 && npc.ownedBuildingIds.length === 0) {
                    goals.push({
                        ...
                        priority: 0.7,
                        conditions: [
                            { type: 'owns_building', key: 'house', value: true, operator: '==' }
                        ],
                        isGlobal: true,
            ...
            case NPCContextState.STABLE:
                ...
                if (npc.skills.crafting < 50) {
                    goals.push({
                        ...
                        priority: 0.5,
                        conditions: [
                            { type: 'skill_level', key: 'crafting', value: 50, operator: '>=' }
                        ],
                        isGlobal: true,
                        createdAt: currentTick
                    });
        ...
        if (totalInventory > 10) {
            goals.push({
                ...
                priority: 0.4,
                conditions: [
                    { type: 'inventory_count', key: 'total', value: 5, operator: '<=' }
                ],
                isGlobal: false,
                createdAt: currentTick
            });
